...
export default class WhiteScreen {
  ...
  sampleComparison() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      // 中心点只计算一次
      if (i !== centerPoint && this.isContainer(yElement as HTMLElement))
        emptyPoints++;
    }
    ...
    if (emptyPoints !== 19) {
      ...
      if (this.loopCount > maxLoopCount) {
        ...
        _support._loop_while_screen_timer_ &&
          clearInterval(_support._loop_while_screen_timer_);
      } else {
        this.loopCount++;
  ...
  loopWhileScreen() {
    ...
    _support._loop_while_screen_timer_ = setInterval(
      this.sampleComparison.bind(this),
      1000,
    );
  }
