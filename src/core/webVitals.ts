...
export default class WebVitals {
  ...
  getTTFB(callback: Callback) {
    if (this.isWebVitalsSupported()) {
      ...
      if (navigationEntry) {
        const rating =
          navigationEntry.responseStart < 800
            ? RatingEnum.GD
            : navigationEntry.responseStart < 1800
              ? RatingEnum.NI
              : RatingEnum.PR;
  ...
  private isWebVitalsSupported(): boolean {
    return (
      'PerformanceObserver' in _global &&
