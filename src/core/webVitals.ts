...
import {PerformanceEnum, RatingEnum, ReportTypeEnum} from "../types";
        ...
        if (this.isWebVitalsSupported()) {
            onTTFB((metric) => {
                console.info("---metrics report---", {
                    ...
                    data: {name: PerformanceEnum.TTFB, value: metric.value, rating: metric.rating}
                });
            })
        } else {
            ...
            if (navigationEntry) {
                ...
                console.info("---metrics report---", {
                    type: ReportTypeEnum.PERFORMANCE,
                    data: {name: PerformanceEnum.TTFB, value: navigationEntry.responseStart, rating}
                });
            }
        }
    }
    ...
    getFCP() {
        if (this.isWebVitalsSupported()) {
            onFCP((metric) => {
                console.info("---metrics report---", {
                    type: ReportTypeEnum.PERFORMANCE,
                    data: {name: PerformanceEnum.FCP, value: metric.value, rating: metric.rating}
                });
            })
        } else {
            new PerformanceObserver((entryList) => {
                ...
                if (fcpEntry) {
                    ...
                    console.info("---metrics report---", {
                        type: ReportTypeEnum.PERFORMANCE,
                        data: {name: PerformanceEnum.FCP, value: fcpEntry.startTime, rating}
                    });
                }
    ...
    getLCP() {
        if (this.isWebVitalsSupported()) {
            onLCP((metric) => {
                console.info("---metrics report---", {
                    type: ReportTypeEnum.PERFORMANCE,
                    data: {name: PerformanceEnum.LCP, value: metric.value, rating: metric.rating}
                });
            })
        } else {
            new PerformanceObserver((entryList) => {
                ...
                if (lcpEntry) {
                    ...
                    console.info("---metrics report---", {
                        type: ReportTypeEnum.PERFORMANCE,
                        data: {name: PerformanceEnum.LCP, value: lcpEntry.startTime, rating}
                    });
                }
        ...
        new PerformanceObserver((entryList) => {
            entryList.getEntries().forEach((entry) => {
                if (entry.name === "first-contentful-paint") {
                    ...
                    console.info("---metrics report---", {
                        type: ReportTypeEnum.PERFORMANCE,
                        data: {name: PerformanceEnum.FSP, value: entry.startTime, rating}
                    });
                }
            })
        }).observe({type: "paint", buffered: true});
    ...
    getFID() {
        if (this.isWebVitalsSupported()) {
            onFID((metric) => {
                console.info("---metrics report---", {
                    type: ReportTypeEnum.PERFORMANCE,
                    data: {name: PerformanceEnum.FID, value: metric.value, rating: metric.rating}
                });
            ...
            new PerformanceObserver((entryList, po) => {
                entryList.getEntries().forEach(entry => {
                    ...
                    console.info("---metrics report---", {
                        type: ReportTypeEnum.PERFORMANCE,
                        data: {name: PerformanceEnum.FID, value: fid, rating}
                    });
                })
    ...
    getCLS() {
        if (this.isWebVitalsSupported()) {
            onCLS((metric) => {
                console.info("---metrics report---", {
                    type: ReportTypeEnum.PERFORMANCE,
                    data: {name: PerformanceEnum.CLS, value: metric.value, rating: metric.rating}
                });
            })
        } else {
            ...
            new PerformanceObserver((list) => {
                for (const entry of list.getEntries()) {
                    ...
                    if (!layoutShiftEntry.hadRecentInput) {
                        ...
                        console.info("---metrics report---", {
                            type: ReportTypeEnum.PERFORMANCE,
                            data: {name: PerformanceEnum.CLS, value: layoutShiftEntry.value, rating}
                        });
                    }
                }
